<template>
  <div class="relative">
    ...
    <div class="relative h-56 overflow-hidden rounded-lg sm:h-64 xl:h-80 2xl:h-96">
      ...
      <div v-for="(picture, index) in pictures" :key="index" :class="index === currentPicture ? 'z-0' : '-z-1'"
    ...
    <!-- Slider indicators -->
    <div v-if="!noIndicators" class="absolute bottom-5 left-1/2 z-30 flex -translate-x-1/2 space-x-3">
      <button v-for="(picture, index) in pictures" :key="index" :aria-label="'Slide ' + index"
        ...
        class="size-3 rounded-full bg-white" type="button" @click.prevent="slideTo(index)" />
    </div>
    <!-- Slider controls -->
    <template v-if="!noControls">
      <button
        class="group absolute left-0 top-0 z-30 flex h-full cursor-pointer items-center justify-center px-4 focus:outline-none"
        data-carousel-prev type="button" @click.prevent="previousPicture">
        <span
          ...
          <svg class="size-5 text-white dark:text-gray-800 sm:size-6" fill="none" stroke="currentColor"
      ...
      <button
        ...
        data-carousel-next type="button" @click.prevent="nextPicture">
        <span
          class="inline-flex size-8 items-center justify-center rounded-full bg-white/30 group-hover:bg-white/50 group-focus:outline-none group-focus:ring-4 group-focus:ring-white dark:bg-gray-800/30 dark:group-hover:bg-gray-800/60 dark:group-focus:ring-gray-800/70 sm:size-10">
          <svg class="size-5 text-white dark:text-gray-800 sm:size-6" fill="none" stroke="currentColor"
            viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
