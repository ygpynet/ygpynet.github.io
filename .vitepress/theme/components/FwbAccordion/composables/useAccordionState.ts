...
const useAccordionState: UseAccordionState = (
  ...
  const getAccordionState: GetAccordionState = ({ element }) => {
    const accordionId =
      element.value &&
      (element.value.closest("[data-accordion-id]") as HTMLElement)?.dataset
        .accordionId;
  ...
  const getAccordionPanelState: GetAccordionPanelState = ({
    accordionState,
    panelId,
  }) =>
    accordionState?.value.panels.find(
      (panel: AccordionPanel) => panel.id === panelId,
    ) ?? null;
