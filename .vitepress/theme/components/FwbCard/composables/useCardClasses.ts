...
export function useCardsClasses(props: UseCardsClassesProps): {
  ...
  const cardClasses = computed(() => {
    ...
    if (props.variant.value === "image") {
      computedClasses =
        "min-w-sm rounded-lg border border-gray-200 shadow-md dark:border-gray-700";
    } else if (props.variant.value === "default") {
      computedClasses =
        "block rounded-lg border border-gray-200 shadow-md dark:border-gray-700 ";
    } else if (props.variant.value === "horizontal") {
      computedClasses =
        "flex flex-col items-center rounded-lg border shadow-md md:flex-row md:min-w-xl dark:border-gray-700";
    }
  ...
  const horizontalImageClasses = computed(() =>
    props.variant.value === "horizontal"
